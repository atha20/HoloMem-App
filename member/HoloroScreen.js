...
function HoloroScreen({ navigation }) {
    const [data, setData] = useState([]);

    useEffect(() => {
        const fetchData = async () => {
            await axios
            .get('https://api.npoint.io/c8fbf3c68c02d5c303f3/holoidgen2')
    ...
    return (
        <View style={styles.container}>
            ...
            <FlatList
                ...
                data={data}
                renderItem={({ item }) => (
                    <TouchableOpacity onPress={() => navigation.navigate('Detail holoro', { msg: item.id })}>
